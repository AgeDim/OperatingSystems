...
static void fill_structs(void){
 ...
 msg->si = vmalloc(sizeof(struct syscall_info));
 msg->ctime = vmalloc(sizeof(struct task_cputime));
	
 tsk = get_pid_task(find_get_pid(pid), PIDTYPE_PID);
	
 msg->ctime->stime = tsk->stime;
 msg->ctime->utime = tsk->utime;
 msg->ctime->sum_exec_runtime = tsk->stime + tsk->utime;

 unsigned long args[6]={ };
 struct pt_regs* regs = task_pt_regs(tsk);
 msg->si->sp = user_stack_pointer(regs);
 msg->si->data.instruction_pointer = instruction_pointer(regs);
 msg->si->data.nr = syscall_get_nr(tsk, regs);
 if(msg->si->data.nr != -1L) syscall_get_arguments(tsk, regs, args);
}
